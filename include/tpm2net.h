...
#define TPM2_NET_PACKET_TOTAL_PACK_NUM_P \
    0x05  // The total nummber of packets to be transfered for one frame

#define TPM2_SER_DATA_P 0x04  // Data starts at 5th position
#define TPM2_NET_DATA_P 0x06  // Data starts at 7th position
#define TPM2_SER_CMD_CONTROL_P \
    0x04  // If block is a 'COMMAND BLOCK' than 5th position contains the 'COMMAND CONTROL BYTE'
          // (described below)
#define TPM2_NET_CMD_CONTROL_P \
    0x06  // If block is a 'COMMAND BLOCK' than 7th position contains the 'COMMAND CONTROL BYTE'
          // (described below)
#define TPM2_SER_CMD_TYPE_P \
    0x05  // If block is a 'COMMAND BLOCK' than the actual command type is on 6th position
#define TPM2_NET_CMD_TYPE_P \
    0x07  // If block is a 'COMMAND BLOCK' than the actual command type is on 8th position
#define TPM2_SER_SP_CMD_P \
    0x06  // If command type is 'SPECIAL COMMAND' than the actual special command is on 7th position
#define TPM2_NET_SP_CMD_P \
    0x08  // If command type is 'SPECIAL COMMAND' than the actual special command is on 9th position
#define TPM2_SER_CMD_DATA_P \
    0x06  // If block is a 'COMMAND BLOCK' than command data starts at 7th position
#define TPM2_NET_CMD_DATA_P \
    0x08  // If block is a 'COMMAND BLOCK' than command data starts at 9th position
#define TPM2_SER_SP_CMD_DATA_P \
    0x07  // If block is a 'COMMAND BLOCK' and command type is 'SPECIAL COMMAND' than command data
          // starts at 8th position
#define TPM2_NET_SP_CMD_DATA_P \
    0x09  // If block is a 'COMMAND BLOCK' and command type is 'SPECIAL COMMAND' than command data
          // starts at 10th position
...
#define TPM2_CMD_SPECIAL_CMD \
    0xFF  // SPECIAL COMMAND --> In this case user can define any own command on position
          // 'TPM2_SP_CMD_P'
...
#define TPM2_CMD_DIR_BIT \
    0x07  // The MSB of the 'CONTROL BYTE' determines whether the command is a READ command or a
          // WRITE command
#define TPM2_CMD_DIR_READ 0x00   // For a READ  command it is 0
#define TPM2_CMD_DIR_WRITE 0x01  // For a WRITE command it is 1
#define TPM2_CMD_ACK_BIT \
    0x06  // The MSB-1 of the 'CONTROL BYTE' determines whether the command expects an answer or not